...
file_formatter = ColoredFormatter('[%(asctime)s.%(msecs)03d] %(levelname)s: %(message)s',
                                  datefmt='%Y-%m-%d %H:%M:%S', colored=False)
...
def get_time(date_type:str = None):
    ...
    if date_type == "text":
        return (time_now.strftime("[%I:%M%p %Y/%m/%d] (UTC+0)")
...
def server_status():
    ...
    return FN_JSON["msg"]["en"]["server.status"].format(
            cpu_used, cpu_cores, cpu_freq, ram_usedp, ram_used,
            ram_total, disk_usedp, disk_used, disk_total, uptime_str
        )
