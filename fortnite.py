...
async def get_access_token(token: str, grant_type="authorization_code"):
    ...
    *refresh_token live 23d
    ...
    res_json = await aiorequest(
        "post", link, headers_, data, from_func="get_access_token"
    )
...
def rename_quests(data: dict, lang: str):
    ...
    for quest_id in data.get("items"):
        ...
        if (
            item_data.get("templateId").lower().startswith("quest:daily_")
            and item_data.get("attributes").get("quest_state").lower() == "active"
        ):
            ...
            for obj in objs:
                ...
                progress += (
                    f"{complete_num}/{objs[obj]['count']} {objs[obj]['name'][lang]},"
                )
            progress = progress[:-1]
            ...
            for rwrd in rewards:
                ...
                if rwrd.startswith("ConditionalResource:"):
                    reward_text += (
                        f"{reward_amount}x {reward_name['PassedConditionItem']},"
                    )
...
async def quest_reroll(quest_id: str, headers: dict, account_id: str):
    ...
    res_json = await aiorequest(
        "post", link, headers, json=json, from_func="quest_reroll"
    )
...
async def tokens_check_and_update(user_id: int):
    ...
    if await check_token_expires(token):
        ...
        if await check_refresh_expires(token):  #! maybe -2h? i don't know
            log.warning(
                f"tokens_check_and_update(), user_id:{user_id}, "
    ...
    await bot.send_message(
        user_id, "All tokens died, wtf? Please use /start for auth-code."
    )
...
async def send_quest_message(user_id, user_data, msg, keyboard):
    ...
    try:
        ...
        log.info(
            f"Edit old quest message user_id: {user_id}, "
            + f"msg_id: {user_data['first_quest_msg']}"
        )
    except Exception:
        bot_msg = await bot.send_message(
            user_id, msg, parse_mode="HTML", reply_markup=keyboard
        )
        log.info(
            f"Send first/new quest message user_id: {user_id}, "
...
async def process_quests(user_id, user_data, pve_data):
    ...
    renamed_quests = await asyncio.to_thread(rename_quests, pve_data, user_data["lang"])
    if renamed_quests:
