...
async def later_del_msg(messages: list = None, time: int = 0):
    ...
    """
    if messages:
        log.info("Delete messages " + f"user_id: {messages[0].chat.id}, time: {time}'s")
...
async def send_auth_link(message: types.Message):
    ...
    bot_msg = await bot.send_message(
        user_id, msg, parse_mode="HTML", disable_web_page_preview=True
    )
    ...
    if not user_data:
        ...
        if user_lang != "en" and bool(FN_JSON["lang"].get(user_lang, 0)):
            ...
            await bot.edit_message_text(
                msg, user_id, bot_msg.message_id, parse_mode="HTML"
            )
        else:
            user_lang = "en"
...
async def gen_confirm_buttons(message: types.Message, button_id: str):
    ...
    await bot.edit_message_reply_markup(
        message.chat.id, message.message_id, reply_markup=confirm_keyboard
    )
...
async def lang_button_click(callback_query: CallbackQuery):
    ...
    bot_msg = await bot.edit_message_text(
        msg, user_id, msg_id, parse_mode="HTML", reply_markup=None
    )
...
async def confirm_button_click(callback_query: CallbackQuery):
    ...
    if confirm == "yes":
        ...
        if await quest_reroll(quest_id, headers, account_id):  # request
            await bot.answer_callback_query(callback_query.id, "🔥Quest rerolled!🔥")
        ...
        await bot.edit_message_reply_markup(
            user_id, msg_id, reply_markup=await gen_quest_buttons(user_id)
        )
